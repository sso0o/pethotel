...
public class FileController {
    ...
    // 이미지 파일 제공 엔드포인트
    @GetMapping("/uploads/hotel/{fileName}")
    public ResponseEntity<?> getHotelImageCommon(@PathVariable String fileName) {
        try {
            // 파일 경로 설정
            Path filePath = Paths.get("./uploads/hotel/" + fileName);
            Resource resource = new UrlResource(filePath.toUri());
    ...
    // 이미지 파일 제공 엔드포인트
    @GetMapping("/uploads/room/{fileName}")
    public ResponseEntity<?> getRoomImageCommon(@PathVariable String fileName) {
        try {
            // 파일 경로 설정
            Path filePath = Paths.get("./uploads/room/" + fileName);
            Resource resource = new UrlResource(filePath.toUri());
